...
export function DeleteItemConfirm({ item, onClose, onConfirm }: Props) {
  const { t } = useLanguage();
    ...
    >
      <div className="modal-content w-full max-w-xs rounded-2xl bg-[var(--surface)] p-6 shadow-xl">
        <h3 className="mb-1 text-sm font-bold text-[var(--foreground)]">
          {inv.deleteItemConfirm ?? inv.deleteConfirm}
        </h3>
        <p className="mb-1 text-xs font-semibold text-zinc-500">{item.name}</p>
        <p className="mb-5 text-xs text-zinc-400">
          {inv.deleteItemWarning ?? inv.deleteConfirm}
        </p>
